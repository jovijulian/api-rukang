...
class ShelfController extends Controller
{
    ...
    public function index(Request $request)
    {
        try {
            ...
            $sort = $request->has('sort') ? $request->input('sort') : 'shelf_name';
            ...
            $countAll = Shelf::query()
                ->count();

            // paging response.
            $response = ShelfResource::collection($paginate);
    ...
    protected function edit($id, array $data, Request $request)
    {
        ...
        // Find shelf by id
        $shelfData = Shelf::find($id);

        if (empty($shelfData)) {
            throw new \Exception("Invalid shelf id", 406);
        }
        $shelfData->id = $id;
        $shelfData->shelf_name = $data['shelf_name'];
        $shelfData->updated_at = $timeNow;
        $shelfData->updated_by = auth()->user()->fullname;
        //Save
        $shelfData->save();
    ...
    public function datatable(Request $request)
    {
        ...
        $order = $columns[$request->has('order.0.column')] ? 'shelf_name'  : $columns[$request->input('order.0.column')];
        $dir = $request->input('order.0.dir');
        //QUERI CUSTOM
        $totalData = Shelf::count();
        if (empty($request->input('search.value'))) {
            //QUERI CUSTOM
            $data = Shelf::offset($start)->limit($limit)->orderBy($order, $dir)->get();
            $totalFiltered = $totalData;
        } else {
            $search = $request->input('search.value');
            $conditions = '1 = 1';
            if (!empty($search)) {
                $conditions .= " AND shelf_name LIKE '%" . trim($search) . "%'";
                $conditions .= " OR created_by LIKE '%" . trim($search) . "%'";
                $conditions .= " OR updated_by LIKE '%" . trim($search) . "%'";
            }
            //QUERI CUSTOM
            $data =  Shelf::whereRaw($conditions)
                ->offset($start)
                ->limit($limit)
                ->orderBy($order, $dir)
                ->get();
            ...
            //QUERI CUSTOM
            $totalFiltered = Shelf::whereRaw($conditions)->count();
        }
