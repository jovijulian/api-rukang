  ...
  <div class="cardhead">
    <div class="content container-fluid">
      ...
      <div class="page-header">
        <div class="row">
          <div class="col">
            <h3 class="page-title">Ubah Status</h3>
            <ul class="breadcrumb">
              <li class="breadcrumb-item"><a href="{{ url('/') }}">Dashboard</a></li>
              <li class="breadcrumb-item"><a href="{{ url('/product') }}">Produk</a></li>
              <li class="breadcrumb-item active">Ubah Status Produk</li>
            </ul>
          </div>
        </div>
      </div>
      ...
      function getProduct() {
        axios.get("{{ url('api/v1/product/detail/' . $idProduct) }}", config)
          .then(res => {
            ...
            $("#status-product").append(`<option value=${status.status_id} selected>${status.status_name}</option>`)
            $("#shipping").append(`<option value=${status.shipping_id} selected>${status.shipping_name}</option>`)
            $('#number-plate').val(status.number_plate)
            $('#current-location').val(location.current_location)
            $('#note').val(status.note)
            
            // image = [
            //   status.status_photo,
            //   status.status_photo2,
            //   status.status_photo3,
            //   status.status_photo4,
            //   status.status_photo5,
            //   status.status_photo6,
            //   status.status_photo7,
            //   status.status_photo8,
            //   status.status_photo9,
            //   status.status_photo10,
            // ]
