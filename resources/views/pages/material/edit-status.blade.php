...
@section('content')
        ...
        <div class="row">
          <div class="col">
            ...
            <ul class="breadcrumb">
              ...
              <li class="breadcrumb-item active">Ubah Foto Status Bahan</li>
            </ul>
          </div>
        </div>
      </div>
  ...
  <script>
    $(document).ready(function() {
      ...
      function getProduct() {
        axios.get("{{ url('api/v1/material/detail/' . $idProduct) }}", config)
          .then(res => {
            const product = res.data.data.item
            const status = product.status_material_logs.find(stat => stat.id === '{{ $idStatus }}')
            const location = product.location_material_logs.find(loc => loc.status_material_log_id === '{{ $idStatus }}')
      ...
      $('#update-status-form').on('submit', () => {
        ...
        axios.post("{{ url('api/v1/material/multiple-image-status/' . $idStatus) }}", data, config)
          .then(res => {
            const produk = res.data.data.item
            sessionStorage.setItem("success", `Status bahan berhasil diubah`)
            window.location.href = `{{ url('/material/detail/${res.data.data.item.material_id}') }}`
          })
          .catch(err => {
            $('#global-loader').hide()
            Swal.fire('Status bahan gagal diubah', '', 'error')
            console.log(err)
          })
