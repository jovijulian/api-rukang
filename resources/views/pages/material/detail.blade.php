...
@section('content')
  <div class="content">
    <div class="page-header">
      ...
      <div class="page-btn">
        <a href="/tool/update-status" class="btn btn-added update-status remove-role">Update Status Bahan</a>
      </div>
    </div>
    <!-- /add -->
    <div class="row">
      ...
      axios.get("{{ url('api/v1/material/detail/' . $id) }}", config)
        .then(res => {
              ...
              `
                <tr>
                  <td>
                    ...
                    <a href="/material/edit-status/${product.id}/${statusLog.id}" class="p-2 btn btn-submit text-white" ${hiddenRole && 'hidden'}>Ubah foto status</a>
                  </td>
          ...
          product.location_mmaterial_logs.map((locationLog, i) => {
            const link = `<a href="{{ url('material/edit-location/` + locationLog.status_log_id + `') }}" class='p-2 btn btn-submit text-white' ${hiddenRole && 'hidden'}>Update Lokasi</a>`
